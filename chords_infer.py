...
import constants
from note_sequence import NoteSequence, KeySignature, AnnotationType, TextAnnotation
import note_sequence_ops
import numpy as np
...
def _key_chord_distribution(chord_pitch_out_of_key_prob):
    ...
    for key in range(12):
        ...
        for i, chord in enumerate(_CHORDS[1:]):
            ...
            num_pitches_out_of_key[key,
                                   i + 1] = len(chord_pitches - key_pitches)
            ...
            if key_1 != key_2:
                ...
                if chord_1 != chord_2:
                    ...
                    mat[i, j] *= (chord_change_prob * (
                            ...
                            key_chord_distribution[key_2, chord_index_1] / (len(_CHORDS) - 1)))
                else:
                    # No chord change.
...
def sequence_note_pitch_vectors(sequence, seconds_per_frame, lowest_pitch_weight=2.):
    ...
    lowest_pitch = np.ones(num_frames, dtype=np.int) * 1000
    ...
    for note in sequence.notes:
        ...
        for frame in range(start_frame, end_frame + 1):
            lowest_pitch[frame] = min(int(lowest_pitch[frame]), int(note.pitch))
    ...
    #     x[frame, lowest_pitch[frame]] *= lowest_pitch_weight
...
def infer_chords_for_sequence(sequence,
    ...
    Raises:
      ...
      SequenceTooLongError: If the number of chords to be inferred is too
          large.
    """
    assert sequence.quantization_info.steps_per_quarter > 0
    ...
    steps_per_bar_float = note_sequence_ops.steps_per_bar_in_quantized_sequence(sequence)
    steps_per_chord_float = steps_per_bar_float / chords_per_bar
    if steps_per_chord_float != round(steps_per_chord_float):
        raise NonIntegerStepsPerChordError('Non-integer number of steps per chord: %f' %
                                           steps_per_chord_float)
    steps_per_chord = int(steps_per_chord_float)
    steps_per_second = note_sequence_ops.steps_per_quarter_to_steps_per_second(
    ...
    note_pitch_vectors = sequence_note_pitch_vectors(sequence, seconds_per_chord)  # TODO: save this vector
    ...
    for frame, (key, chord) in enumerate(key_chords):
        ...
        if _PITCH_CLASS_NAMES[key] != current_key_name:
            ...
            if add_key_signatures:
                ks = KeySignature(time, key, key // 12)
                sequence.key_signatures.append(ks)
            else:
                if current_key_name is not None:
                    print('Sequence has key change from %s to %s at %f seconds.',
        ...
        ta = TextAnnotation(time, frame * steps_per_chord, figure, AnnotationType.CHORD_SYMBOL, chord[0], chord[1],
                            note_pitch_vectors[frame])
        sequence.text_annotations.append(ta)
