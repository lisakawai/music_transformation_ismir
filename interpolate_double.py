...
def test(f_chord_test, f_style_test, f_event_test, keys_test, model, device, attrs,
    ...
    mul_tgt = [-1.0, -0.5, -0.3, 0.3, 0.5, 1.0]
    vec_to_interpolate_mul = to_float_tensor(np.array([[i, j] for i in mul_tgt for j in mul_tgt]), device=device)
    ...
    with torch.no_grad():
        for key in keys_test:
            ...
            style_tensor[interpolate_num: 2 * interpolate_num, 1] += vec_to_interpolate
            style_tensor[interpolate_num * 2: interpolate_num * 2 + len(mul_tgt) ** 2] += vec_to_interpolate_mul
            ...
            names = ['{}_{}_{:.5}'.format(key, attrs[0], original_style_value_0 + (i - 5) * 0.1) for i in range(interpolate_num)] +\
                    ['{}_{}_{:.5}'.format(key, attrs[1], original_style_value_1 + (i - 5) * 0.1) for i in range(interpolate_num)] +\
                    ['{}_demo_{}_{:.5}_{}_{:.5}'.format(
                        key, attrs[0], original_style_value_0 + i, attrs[1], original_style_value_1 + j)
                        for i in mul_tgt for j in mul_tgt]
